...
class ApiService {
  ...
  Future<List<Office>> getOffices() async {
    try {
      ...
      if (data['success'] != true) {
        throw Exception(data['message'] ?? 'فشل في تحميل المكاتب');
      }
  ...
  Future<List<Representation>> getRepresentations() async {
    try {
      ...
      if (data['success'] != true) {
        throw Exception(data['message'] ?? 'فشل في تحميل الممثليات');
      }
  ...
  Future<Representation> getRepresentationDetails(String id) async {
    try {
      ...
      if (data['success'] != true) {
        throw Exception(data['message'] ?? 'فشل في تحميل التفاصيل');
      }
  ...
  Future<List<Region>> searchRegions(String query) async {
    try {
      ...
      if (data['success'] != true) {
        throw Exception(data['message'] ?? 'فشل في البحث');
      }
